...
if [[ "$UI_FRAMEWORK" == "streamlit" ]]; then
  ...
  exec streamlit run src/app.py --server.address "$UI_HOST" --server.port "$UI_PORT" --server.headless true --server.enableCORS false --server.enableXsrfProtection false
else
